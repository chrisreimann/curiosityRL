...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def CuriosityReward(self, obs):
        
        state = tuple(obs.flatten())
        if state in self.seenObs:
            #print("observations already seen")
            self.seenObs[state] += 1
        
        else:
            self.seenObs[state] = 1
        
        #return 1/math.sqrt(self.seenObs[state])
        return 1/self.seenObs[state]
    ...
    def step(self, action: ActType) -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        # Curiosity reward
        reward += self.CuriosityReward(observation)
        #if terminated or truncated:
        #    self.seenObs = {}
        #print("found reward (monitor): ", reward)
